...
**v1.6.5 リリース**: ドラッグ＆ドロップ（D&D）機能を伴うリスト表示の共通化（`DraggableList` / `DraggableItem`）を行い、ブックマーク一覧のリファクタリングを完了しました。これにより、複雑な並び替えロジックの再利用性とコードの保守性が向上しています。
...
- **詳細表示・編集画面 (v1.6.5)**: 行を選択することで専用の詳細ページへ遷移します。
