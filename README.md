...
**v1.0.1 リリース**: ブックマークの CRUD 操作、ドラッグ＆ドロップによる並び替え、およびブラウザ拡張機能といった、ブックマーク管理に最低限必要な基本機能が一通り揃いました。定数管理の最適化と UI の一貫性向上による品質改善も行われています。
...
- **詳細表示・編集画面 (v1.5.0)**: 行を選択することで専用の詳細ページへ遷移します。現在はプレースホルダですが、`Enter` キーでブックマークを直接開く、`ESC` キーで一覧に戻る（選択解除）といったキーボードショートカットにより、快適な操作が可能です。
