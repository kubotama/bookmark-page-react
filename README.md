...
**v1.9.1 リリース**: キーワード選択中に **Enter キーで一致したブックマークをすべて一括で開く機能**を追加しました。特定のコンテキストに関連するサイト群を瞬時に起動できるようになり、作業効率が向上します。

**v1.9.0 リリース**: 選択したキーワードに基づき、ブックマークを「一致するもの」と「それ以外」に**上下に分けて表示する機能**を実装しました。
